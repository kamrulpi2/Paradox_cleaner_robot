...
<head>
  ...
  <style>
    * {
    ...
    .btn-mode   { background: #475569; }
    .btn-mode.active { background: #38bdf8; }
...
<body>
  ...
  <div class="card">
    ...
    <div class="btn-row">
      <button class="btn btn-mode active" id="modeManualBtn" onclick="setMode('MANUAL')">🕹️ Manual</button>
      <button class="btn btn-mode" id="modeAutoBtn" onclick="setMode('AUTO')">🤖 Auto (HuskyLens)</button>
    </div>
    <div class="status">
      Status: <span id="autoStatus">-</span> | Bottles caught: <span id="bottleCount">0</span>
    </div>
  </div>

  <!-- SERVO -->
  <div class="card">
    <div class="card-title">Servos (Mirrored / Claw)</div>
  ...
  <div class="card">
    <div class="card-title">Robot Movement (Manual mode only)</div>
  ...
  <script>
    ...
    function updateModeUI() {
      document.getElementById('modeManualBtn').classList.toggle('active', currentModeUI === 'MANUAL');
      document.getElementById('modeAutoBtn').classList.toggle('active', currentModeUI === 'AUTO');
      document.getElementById('moveGrid').classList.toggle('disabled', currentModeUI === 'AUTO');
    }

    // ============ STATUS POLLING ============
    function pollStatus() {
      fetch('/status').then(r => r.text()).then(t => {
        // format: mode=AUTO,status=APPROACHING,bottles=2,camX=140,camW=95
        const parts = {};
        t.split(',').forEach(p => {
          const kv = p.split('=');
          parts[kv[0]] = kv[1];
        });
        if (parts.mode && parts.mode !== currentModeUI) {
          currentModeUI = parts.mode;
          updateModeUI();
        }
        document.getElementById('autoStatus').innerText = parts.status || '-';
        document.getElementById('bottleCount').innerText = parts.bottles || '0';
      });
    }
