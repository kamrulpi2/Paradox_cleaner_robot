...
// ---- Mode switch: /mode?m=AUTO or /mode?m=MANUAL ----
void handleModeSet() {
  if (server.hasArg("m")) {
    ...
    if (m == "AUTO") {
      ...
      motorStop();   // safety stop whenever leaving auto mode
    }
  }
  server.send(200, "text/plain", currentMode == MODE_AUTO ? "AUTO" : "MANUAL");
}

// ---- Status for the web page (mode, HuskyLens status, bottle count) ----
void handleStatus() {
  String msg = "mode=";
  msg += (currentMode == MODE_AUTO ? "AUTO" : "MANUAL");
  msg += ",status=" + autoStatus;
  msg += ",bottles=" + String(bottlesCaught);
  msg += ",camX=" + String(lastCamX);
  msg += ",camW=" + String(lastCamW);
...
void setup() {
  ...
  // HuskyLens (Object Tracking). Manual mode still works even if the
  // camera is not connected - only AUTO mode needs it.
  initHuskyLens();
